...
8. **Ship the task** — `/kill-this` commits, pushes, opens PR with `closes #<issue>`, appends a `## Task <N>` block to the session file (on the orphan `sessions` branch). Run per task; multiple per session. Hand-typing the `git push` + `gh pr create` is **not** the same thing: it skips `@code-review`, which is the only automatic read of the diff before it merges. The session log gap is cheap; the unreviewed merge is not.
