...
- **`context`** — file is project-specific. Drop the pair from diff scope entirely. Record in Step 6 aggregated summary (see Step 6 below). **Do not emit a Step 3 table row.** Two kinds of file land here: per-project *documents* (`docs/SPEC.md`, `docs/DECISIONS.md`, …) and the **project-owned reviewers** — `agents/architect.md`, `agents/code-review.md`, `agents/ui-reviewer.md` (DEC-S035). The reviewers reason about the project's own substance, so seeds' copies are install-time starting points only; never forward-port into them, in either direction or either mode, however stale the project's copy looks. An adapted reviewer that diverges heavily from the template is working as designed, not drifting.
