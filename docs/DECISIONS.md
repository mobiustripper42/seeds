...
**What changes:**
- **This slice (landed):** `/retro` Step 2 fully rewritten (throughput + estimate-calibration tally; no transcript) in **both** skill copies — the template (`dev/claude/skills/retro/SKILL.md`) and seeds' dogfooded live install (`.claude/skills/retro/SKILL.md`), which had drifted onto the older DEC-S015 model and is now reconverged byte-identical. Downstream Steps 3/4/5.5/6/7/8/10 + Notes updated off active/breaks. `velocity.py` → `throughput.py`: reads GitHub (issue dates+labels, merged PRs) instead of RETROSPECTIVES.md; reports the two lifetime rates, per-phase points + pointing-stability (pts/issue) + span, and **PR cycle time (open→merge) by point size** as a median/p85 distribution — the "if I start it, when's it done?" forecast — plus a drift-proof issue-count companion via `--issues`.
