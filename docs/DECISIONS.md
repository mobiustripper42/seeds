...
- DEC-S044 — `drift.mjs` gains a `presence` class — report the absence, never the contents _(refined by DEC-S046 — S044's holding stands untouched — the `presence` class, its semantics and its sole member are unchanged, and nothing here operates on that branch. What is refined is its reach: S044 treated one unclassified file by classifying it, leaving intact the mechanism that made it invisible, and that mechanism produced two more instances)_
