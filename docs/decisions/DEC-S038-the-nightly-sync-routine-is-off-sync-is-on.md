...
- Amends DEC-S004 — the upstream leg — both directions are now manual; downstream-is-manual was already right
- Retires DEC-S010 — the nightly automation only — the config, the classifier and the file-class registry it defined are still read by manual sync
- Retires DEC-S028 — the digest is dormant while the Routine that emitted it is off

**See also** — later decisions that changed part of this one:
- Revised by DEC-S040 — the holding that manual /push-seeds and /pull-seeds carry the steady state — both are now retired; the Routine stays off
