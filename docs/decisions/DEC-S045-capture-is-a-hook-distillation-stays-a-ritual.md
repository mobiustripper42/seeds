...
- **`jq` is a hard dependency.** Without it the script exits silently, because a `SessionEnd` hook cannot *fail* a session — exit 2 only prints to stderr — so every failure here is silent by construction. The only signal that capture works is the queue filling up, which is why the README says to check it.
- **"Cannot block" is a weaker claim than it sounds, and the script says so.** Ignoring exit codes is not the same as being unable to hang. A `cp` or `git` against a stalled network filesystem — OneDrive Files-On-Demand, a hard-mounted NFS home — can enter uninterruptible sleep, which no timeout and no signal reclaims. `timeout 10` wraps the git calls to bound the ordinary slow-disk case; nothing bounds that one, and the windows laptop's checkout living under OneDrive makes it the likeliest place to meet it.
- **A repeat fire leaves the copy current and the index line stale.** The transcript is overwritten; `branch`/`sha`/`reason` still describe the first fire. Rewriting a line in place would mean a read-modify-write on a file the drain is also editing, which is a worse trade than slightly stale metadata on a rare path.
