...
6. **Generate + gate.** `npm run gen:decisions`, then wire `check:decisions && check:context && check:docs` into `verify` ahead of typecheck/test/build. **Expect `check:docs` to go red on the first run** — the roster check almost always finds a skill or agent missing from `CHEATSHEET.md`. That is the ratchet working; fix the docs, don't loosen the check.
