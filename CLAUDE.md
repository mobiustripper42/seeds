...
**The deny list now protects itself.** `Edit(.claude/settings.json)` and the two siblings mean a session cannot quietly remove its own restrictions — the obvious hole in any policy the restricted party can rewrite. You edit those files; that is the DEC-S023 model anyway.
