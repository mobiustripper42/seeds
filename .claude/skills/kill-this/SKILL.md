...
3. Open a PR: `gh pr create --base main --head <branch> --title "<commit subject>" --body "<auto-generated body>"`. Body should include:
...
**Fallback if `gh` is unavailable or not authenticated:**
Push-only (`git push -u origin <branch>`) and tell the user to open the PR manually. Note the missing PR in the draft log's Context.
